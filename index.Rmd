...
#base        <- readxl::read_excel("Data/Base_Motor_familia.xlsx")
base        <- readxl::read_excel("Data/PRUEBAS/Base_motor.xls")
...
```{r}
base_aux <- base[base$CODIND == 500,]
base_aux <- base_aux[base_aux$URBANORURALUY == "Total país",]
max<- max(base_aux$FECHA)
min<- min(base_aux$FECHA)
periodo <- c(min," - ", max)
periodo_formatted <- paste(
  text_spec(periodo, color = 1, font_size = 16, align = "justify"),
  collapse = " ")

VALOR1 <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares según tipo de hogar (Biparental)"&
                     base_aux$SEXO == "Todos"&
                     base_aux$ASCENDENCIA == "Todos"&
                     base_aux$QUINTIL == "Todos"&
                     base_aux$EDAD=="Todos"&
                     base_aux$POBRE=="Todos"&
                     base_aux$FECHA==max,]
VALOR1 <- round(as.numeric(c(VALOR1$VALOR))*100,1)

VALOR2 <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares según tipo de hogar (Unipersonal)"&
                     base_aux$SEXO == "Todos"&
                     base_aux$ASCENDENCIA == "Todos"&
                     base_aux$QUINTIL == "Todos"&
                     base_aux$EDAD=="Todos"&
                     base_aux$POBRE=="Todos"&
                     base_aux$FECHA==max,]
VALOR2 <- round(as.numeric(c(VALOR2$VALOR))*100,1)

VALOR3 <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares según tipo de hogar (Pareja sin hijos)"&
                     base_aux$SEXO == "Todos"&
                     base_aux$ASCENDENCIA == "Todos"&
                     base_aux$QUINTIL == "Todos"&
                     base_aux$EDAD=="Todos"&
                     base_aux$POBRE=="Todos"&
                     base_aux$FECHA==max,]
VALOR3 <- round(as.numeric(c(VALOR3$VALOR))*100,1)

VALOR4 <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares según tipo de hogar (Monoparental femenino)"&
                     base_aux$SEXO == "Todos"&
                     base_aux$ASCENDENCIA == "Todos"&
                     base_aux$QUINTIL == "Todos"&
                     base_aux$EDAD=="Todos"&
                     base_aux$POBRE=="Todos"&
                     base_aux$FECHA==max,]
VALOR4 <- round(as.numeric(c(VALOR4$VALOR))*100,1)
...
En `r max` se observa que `r VALOR1`% de los hogares están compuestos por una pareja con hijos/as, `r VALOR2`% son unipersonales, `r VALOR3`% compuestos por una preja sin hijos/as y `r VALOR4`% son monoparentales femeninos. En una menor proporción se encuentran los otros tipos de hogares. 
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, fill=UNIDAD)) +
  geom_bar(dat = dat %>% filter(FECHA <= 2019),
            size = 1, stat="identity", alpha = 0.4, width=0.6) +
  geom_bar(dat = dat %>% filter(FECHA > 2019),
            size = 2, stat="identity", alpha = 0.8, width=0.6, show.legend = FALSE) +
  theme_minimal() +
...
#  scale_alpha_manual(values = c(.8, .8, .8, .4)) +
  scale_fill_manual(
    values = c("darkorange", 639,"#a1dd70","darkseagreen3","grey60", "darksalmon","indianred2", "indianred"),
    labels = c("Unipersonal", "Pareja sin hijos", "Biparental","Monoparental femenino", "Monoparental masculino", "Extendido o compuesto", "Extendido con núcleo monoparental", "Sin núcleo")) +
...
```{r}
adv <- strsplit(paste0(ficha [1,"NOTAS2"])," ")[[1]]
adv_formatted <- paste(
...
```{r}
nota <- strsplit(paste0("Notas: ", ficha [1,"NOTAS1"])," ")[[1]]
...
VALOR1<-base_aux[base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$NOMINDICADOR =="Distribución porcentual de hogares con menores según tipo de hogar (Biparental)"&base_aux$FECHA==max, ]
VALOR1<-round(as.numeric(VALOR1$VALOR)*100,1)

VALOR2<-base_aux[base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$NOMINDICADOR =="Distribución porcentual de hogares con menores según tipo de hogar (Monoparental femenino)"&base_aux$FECHA==max, ]
VALOR2<-round(as.numeric(VALOR2$VALOR)*100,1)


VALOR3<-base_aux[base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$NOMINDICADOR =="Distribución porcentual de hogares con menores según tipo de hogar (Extendido o compuesto)"&base_aux$FECHA==max, ]
VALOR3<-round(as.numeric(VALOR3$VALOR)*100,1)

```


<div style="text-align: justify">
En el siguiente gráfico puede observarse que en `r max` la mayoría de hogares en los que residen menores de 18 años son biparentales (`r VALOR1`%), seguido de hogares monoparentales femeninos (`r VALOR2`%) y de extendidos o compuestos (`r VALOR3`%). 
...
periodo_formatted <- paste(
  text_spec(periodo, color = 1, font_size = 16, align = "justify"),
  collapse = " ")
VALOR1 <- base_aux[base_aux$NOMINDICADOR == "Distribución porcentual de hogares con menores según tipo de hogar (Biparental)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Quintil 1"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR1<-round(as.numeric(VALOR1$VALOR)*100)

VALOR2 <- base_aux[base_aux$NOMINDICADOR == "Distribución porcentual de hogares con menores según tipo de hogar (Biparental)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Quintil 5"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR2<-round(as.numeric(VALOR2$VALOR)*100)
...
VALOR1<- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de hogares con menores de 15 años según tipo de hogar (Biparental)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,]
VALOR1 <- round(as.numeric(c(VALOR1$VALOR))*100,1)


VALOR2<- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de hogares con menores de 15 años según tipo de hogar (Monoparental femenino)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,]
VALOR2 <- round(as.numeric(c(VALOR2$VALOR))*100,1)

VALOR3<- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de hogares con menores de 15 años según tipo de hogar (Extendido o compuesto)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,]
VALOR3 <- round(as.numeric(c(VALOR3$VALOR))*100,1)
...
<div style="text-align: justify">
En el siguiente gráfico puede observarse que en `r max` la mayoría de hogares en los que residen menores de 15 años son biparentales (`r VALOR1`%), seguido de hogares monoparentales femeninos (`r VALOR2`%) y de extendidos o compuestos (`r VALOR3`%). 
...
VALOR1 <- base_aux[base_aux$NOMINDICADOR == "Distribución porcentual de hogares con menores de 15 años según tipo de hogar (Biparental)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Quintil 1"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR1<-round(as.numeric(VALOR1$VALOR)*100)

VALOR2 <- base_aux[base_aux$NOMINDICADOR == "Distribución porcentual de hogares con menores de 15 años según tipo de hogar (Biparental)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Quintil 5"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR2<-round(as.numeric(VALOR2$VALOR)*100)
...
A partir del gráfico es posible observar que entre 2006 y `r max` se produce una disminución del porcentaje de hogares con modelo de proveedor tradicional y, como contracara, un aumento del porcentaje de hogares con modelo de doble carrera. 
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, fill=UNIDAD)) +
  ...
  scale_alpha_manual(values = c(.8, .8, .8, .4)) +
  scale_fill_manual(
    values = c("darkorange", 639,"#a1dd70","darkseagreen3","grey60", "darksalmon"),
    labels = c("Proveedor tradicional", "Proveedor modificado", "Doble carrera","Inversión de roles", "Inversión de roles modificado", "Residual")) +
  scale_x_continuous(breaks=seq(2006, max, 1)) +
  labs(
    color = "",
    title = "",
...
VALOR1<- base_aux[base_aux$NOMINDICADOR == "Distribución porcentual de los hogares biparentales según modelo de pareja (Proveedor tradicional)"&base_aux$URBANORURALUY=="Total país"&base_aux$SEXO=="Todos"&base_aux$ASCENDENCIA=="Todos"&base_aux$QUINTIL=="Quintil 1"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR1 <- round(as.numeric(c(VALOR1$VALOR))*100,1)

VALOR2<- base_aux[base_aux$NOMINDICADOR == "Distribución porcentual de los hogares biparentales según modelo de pareja (Proveedor tradicional)"&base_aux$URBANORURALUY=="Total país"&base_aux$SEXO=="Todos"&base_aux$ASCENDENCIA=="Todos"&base_aux$QUINTIL=="Quintil 5"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR2 <- round(as.numeric(c(VALOR2$VALOR))*100,1)

VALOR3<- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares biparentales según modelo de pareja (Doble carrera)"&base_aux$URBANORURALUY=="Total país"&base_aux$SEXO=="Todos"&base_aux$ASCENDENCIA=="Todos"&base_aux$QUINTIL=="Quintil 1"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR3 <- round(as.numeric(c(VALOR3$VALOR))*100,1)

VALOR4<- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares biparentales según modelo de pareja (Doble carrera)"&base_aux$URBANORURALUY=="Total país"&base_aux$SEXO=="Todos"&base_aux$ASCENDENCIA=="Todos"&base_aux$QUINTIL=="Quintil 5"&base_aux$FECHA==max&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos",]
VALOR4 <- round(as.numeric(c(VALOR4$VALOR))*100,1)
...
Considerando el modelo de proveedor de los hogares según el quintil de ingresos hay dos aspectos a destacar. Por un lado, el porcentaje de hogares con modelo de proveedor tradicional disminuye a medida que aumentan los ingresos de los hogares, pasando de `r VALOR1`% en los hogares del primer quintil de ingresos a `r VALOR2`% en los del quinto quintil. Del otro lado, los hogares con modelo de doble carrera aumentan el porcentaje conforme aumenta el quintil de ingresos (`r VALOR3`% en el primer quintil y `r VALOR4`% en el quinto quintil).   
...
VALORa <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de las personas según estado conyugal (Casada/o)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==min,]
VALORa <-round(as.numeric(VALORa$VALOR)*100)
  
VALORb <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de las personas según estado conyugal (Casada/o)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,]
VALORb <-round(as.numeric(VALORb$VALOR)*100)
...
En el siguiente gráfico se observa que mientras el porcentaje de personas solteras, divorciadas o separadas y viudas se mantiene relativamente estable en el período presentado, el porcentaje de personas casadas disminuye en `r VALOR` puntos porcentuales, mientras el porcentaje de personas en unión libre aumenta en similar magnitud.  
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, fill=UNIDAD)) +
  ...
  scale_fill_manual(
    ...
    labels = c("Unión libre", "Casada/o", "Divorciada/o o separada/o","Viuda/o", "Soltera/o")) +
  scale_x_continuous(breaks=seq(2006, max, 1)) +
  labs(
    color = "",
    title = "",
...
VALOR1 <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares según sexo del jefe/a (Mujer)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==min,] 
VALOR1<- round(as.numeric(VALOR1$VALOR)*100,1)

VALOR2 <- base_aux[base_aux$NOMINDICADOR =="Distribución porcentual de los hogares según sexo del jefe/a (Mujer)"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL == "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,] 
VALOR2<- round(as.numeric(VALOR2$VALOR)*100,1)

dif <- round (VALOR2-VALOR1)
```
...
Es posible observar que el porcentaje de hogares con jefatura femenina aumenta `r dif` puntos porcentuales entre 2006 y `r max`, pasando de `r VALOR1`% a `r VALOR2`%. Por su parte, puede verse que en el localidades mayores a 5.000 habitantes es donde el porcentaje de hogares con jefatura femenina es mayor, seguido por las localidades menores de 5.000 habitabtes y, luego, el área rural dispersa.  
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, fill=UNIDAD)) +
  ...
  scale_alpha_manual(values = c(.8, .8, .8, .4)) +
  scale_fill_manual(
    values = c("darkorange", 639), labels = c("Varón", "Mujer")) +
  scale_x_continuous(breaks=seq(2006, max, 1)) +
  labs(
    color = "",
    title = "",
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, color=UNIDAD)) +
  geom_line(dat = dat %>% filter(FECHA <= 2019),
            size = 1, alpha = 0.4) +
    geom_line(dat = dat %>% filter(FECHA >= 2021),
            size = 1, alpha = 0.8) +
  ...
  scale_color_manual(
    ...
    labels = c("Urbano (menos de 5.000 habitantes)", "Urbano (más de 5.000 habitantes)", "Rural disperso", "Total país")) +
  ylim(0, 75) +
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, color=UNIDAD)) +
  ...
  scale_color_manual(
    ...
    labels = c("Mujeres", "Varones", "Total")
  ) +
  ylim(0, 60) +
...
g1 <- 
  base %>% filter((CODIND==518), 
                  SEXO == "Todos", ASCENDENCIA == "Todos", QUINTIL == "Todos", URBANORURALUY == "Todos") %>%
  mutate(VALOR = round(as.numeric(VALOR)*100,1)) %>% 
...
g1 <- 
  base %>% filter((CODIND==518), 
                  ASCENDENCIA == "Todos", QUINTIL == "Todos", SEXO != "Todos") %>%
  mutate(VALOR = round(as.numeric(VALOR)*100,1)) %>% 
  mutate(UNIDAD = case_when(NOMINDICADOR== "Porcentaje de jóvenes y adolescentes que se fueron de su hogar de origen y volvieron" ~ "Salida del hogar de origen y volvieron", 
                            NOMINDICADOR== "Porcentaje de jóvenes y adolescentes que se fueron de su hogar de origen y no volvieron" ~ "Salida del hogar de origen y no nolvieron", 
                            NOMINDICADOR== "Porcentaje de jóvenes y adolescentes que nunca se fueron de su hogar de origen" ~ "No salida del hogar de origen"))  %>%
...
VALOR1 <-  base_aux[base_aux$NOMINDICADOR == "Tasa específica de fecundidad adolescente"&base_aux$FECHA==max,]  
VALOR1 <-  round(as.numeric(VALOR1$VALOR))  

```

<div style="text-align: justify">

La tasa específica de fecundidad adolescente es un indicador que da cuenta de la evolución del embarazo adolescente. Este indicador presenta una disminución de 10 puntos porcentuales entre 1996 y 2013, pasado de 70,5 a 60,9 cada mil mujeres entre 15 y 19 años. A partir de dicho año y hasta 2021, se asiste a una disminución marcada y sostenida, alcanzando en `r max` el valor de `r VALOR1` cada mil mujeres entre 15 y 19 años de edad.    
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, color=SEXO)) +
  geom_line(dat = dat %>% filter(FECHA <= 2019),
            size = 1, alpha = 0.4) +
  geom_line(dat = dat %>% filter(FECHA >= 2021),
            size = 1, alpha = 0.8) +
  geom_point(dat = dat %>% filter(FECHA > 2019), size = 2, alpha= 0.8, show.legend = FALSE) +
  scale_color_manual(
...
VALOR1 <- base_aux[base_aux$NOMINDICADOR == "Tasa de asistencia a la educación inicial de niños/as de 0 a 2 años"&base_aux$URBANORURALUY == "Total país"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL== "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,]
VALOR1<-round(as.numeric(VALOR1$VALOR)*100,1)

base_aux <- base[base$CODIND == 552,]
VALOR2<- base_aux[base_aux$NOMINDICADOR == "Tasa de asistencia a la educación de niños/as de 3 a 5 años"&base_aux$URBANORURALUY == "Urbano (más de 5.000 habitantes)"&base_aux$SEXO == "Todos"&base_aux$ASCENDENCIA == "Todos"&base_aux$QUINTIL== "Todos"&base_aux$EDAD=="Todos"&base_aux$POBRE=="Todos"&base_aux$FECHA==max,]
VALOR2<-round(as.numeric(VALOR2$VALOR)*100,1) 
...
<div style="text-align: justify">
Los siguientes indicadores dan cuenta de la asistencia a centros de cuidado y educación por parte de niños y niñas entre 0 y 5 años de edad. Es posible observar que tanto la asistencia de niños/as entre 0 y 2 años como entre 3 y 5 años aumenta en los períodos considerados. En el caso de los niños/as de 0 a 2 años puede verse que el porcentaje de asistencia pasa de 27.1% en 2012 a `r VALOR1`% en `r max` Por su parte, el porcentaje de asistencia de niños/as de 3 a 5 años aumenta de 55.7% en 1990 a `r VALOR2`% en `r max`. 
</div>
...
g1<-  ggplot(dat, aes(x = FECHA, y = VALOR, alpha=metodo, color=NOMINDICADOR)) +
  geom_line(dat = dat %>% filter(FECHA <= 2019),
            size = 1, alpha = 0.4) +
  geom_line(dat = dat %>% filter(FECHA >= 2021),
            size = 1, alpha = 0.8) +
...
SEXO == "Todos", ASCENDENCIA == "Todos", QUINTIL == "Todos", EDAD=="Todos", POBRE=="Todos") %>%
  mutate(VALOR = round(as.numeric(VALOR)*100,1)) %>% 
  mutate(UNIDAD = case_when(URBANORURALUY=="Total país" ~ "Todos",
URBANORURALUY=="Montevideo" ~ "Montevideo",
URBANORURALUY=="Urbano (más de 5.000 habitantes)" ~ "Interior urbano"))  %>%
  mutate(UNIDAD = factor(UNIDAD, levels = c("Montevideo", "Interior urbano", "País urbano")))  %>%
  mutate(FECHA = factor(FECHA, levels = c("2013", "2018")))  %>%
  ggplot(aes(x = UNIDAD, y = VALOR, fill=UNIDAD)) +
  geom_bar(stat="identity", width=0.3)+
  scale_fill_manual(
  ...
  facet_wrap(~FECHA) +
  ylim(0, 75) +
  scale_x_discrete(labels = c("Montevideo", "Interior urbano", "País urbano")) +
  theme_minimal() +
  theme(legend.position="none") +
  labs(
    color = "",
...
SEXO == "Todos", ASCENDENCIA == "Todos", QUINTIL == "Todos", EDAD=="Todos", POBRE=="Todos") %>%
  mutate(VALOR = round(as.numeric(VALOR)*100,1)) %>% 
  mutate(UNIDAD = case_when(URBANORURALUY=="Urbano (más de 5.000 habitantes)" ~ "Interior urbanos",
URBANORURALUY=="Montevideo" ~ "Montevideo",
URBANORURALUY=="País urbano" ~ "País urbano"))  %>%
  mutate(UNIDAD = factor(UNIDAD, levels = c("Montevideo", "Interior urbano", "País urbano")))  %>%
  rename(ANIO = FECHA)
t1 <- t1 [,c("NOMINDICADOR","PAÍS", "UNIDAD", "ANIO","VALOR")]
#t1 <- t1[order(t1$SEXO, t1$ANIO), ]
...
SEXO == "Todos", ASCENDENCIA == "Todos", QUINTIL == "Todos", EDAD=="Todos", POBRE=="Todos") %>%
  mutate(VALOR = round(as.numeric(VALOR)*100,1)) %>% 
  mutate(UNIDAD = case_when(URBANORURALUY=="País urbano" ~ "País urbano",
URBANORURALUY=="Montevideo" ~ "Montevideo",
URBANORURALUY=="Urbano (más de 5.000 habitantes)" ~ "Interior urbano"))  %>%
  mutate(UNIDAD = factor(UNIDAD, levels = c("Montevideo", "Interior urbano", "País urbano")))  %>%
  mutate(FECHA = factor(FECHA, levels = c("2013", "2018")))  %>%
...
ggplot(aes(x = FECHA, y = VALOR)) +
  ...
  scale_color_manual(
    values = c("darkorange", 639)) +
  ylim(0, 40) +
  scale_x_continuous(breaks=seq(1984, 2023, 5), limits=c(1984,2023))  +
  theme_minimal() +
  labs(
    color = "",
    title = "",
