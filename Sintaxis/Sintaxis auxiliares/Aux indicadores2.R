...
var_int <- c("estado_civil1", "estado_civil2", "estado_civil3", "estado_civil4", "estado_civil5")
b_sem <- sapply(sem2_implant_svy$variables %>% select(var_int), function(x){
  sem2_implant_svy %>%
    srvyr::summarise(stat = srvyr::survey_mean(x, na.rm = TRUE)) 
})
...
# Región
b_reg <- function(y) {
  base <- subset(sem2_implant_svy, bd_region == y) 
  resultados  <-  sapply(base$variables %>% select(var_int), function(x){
    base %>%
      srvyr::summarise(stat1 = srvyr::survey_mean(x, na.rm = TRUE))
...
b_edad <- function(y) {
  base <- subset(sem2_implant_svy, tramo_edad2 == y) 
  resultados  <-  sapply(base$variables %>% select(var_int), function(x){
    base %>%
      srvyr::summarise(stat1 = srvyr::survey_mean(x, na.rm = TRUE))
...
b_pobre <- function(y) {
  base <- subset(sem2_implant_svy, pobre_aux == y) 
  resultados  <-  sapply(base$variables %>% select(var_int), function(x){
    base %>%
      srvyr::summarise(stat1 = srvyr::survey_mean(x, na.rm = TRUE))
