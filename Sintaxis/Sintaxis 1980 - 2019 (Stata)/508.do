...
* INDICADOR: Distribución porcentual de los hogares según sexo del jefe/a
* CÓDIGO:    508
...
**MODIFICACIÓN MANUAL: CÓDIGOS Y LOCAL J PARA VARIABLES AUXILIARES**
    ...
    *TOTAL PAÍS
	local i      = 1
	local codind = 508                  // Código de indicador
	...
	matrix colnames  MATR= VALOR AUXILIAR ANIO SEXOJEFE
	...
	ta bc_pe2 if bc_pe2!=., gen(sexo) 
	
	
	foreach x of numlist 1/2 {
	mean sexo`x' [aw=bc_pesoan] if bc_pe4==1
	matrix MATR  [`i',1]= e(b)
	matrix MATR  [`i',2]=  0
...
/*NO se procesa país urbano para no duplicar valores*/
	...
	foreach x of numlist 1/2 {
	mean sexo`x' [aw=bc_pesoan] if bd_region==`val'&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
	matrix MATR  [`i',3]=  `anio'
	matrix MATR  [`i',4]=  `x'
    ...
    *
	...
	foreach x of numlist 1/2 {
	mean sexo`x' [aw=bc_pesoan] if tramo_edad==`val'&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
    ...
    *
	...
	foreach x of numlist 1/2 {
	mean sexo`x' [aw=bc_pesoan] if bd_e29_1==`val'&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
    ...
    *
	...
	foreach x of numlist 1/2 {
	mean sexo`x' [aw=bc_pesoan] if bd_quintilesy==`val'&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
    ...
    *
	...
	foreach x of numlist 1/2 {
	mean sexo`x' [aw=bc_pesoan] if pobre06==`val'&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
...
rename pobre_06 pobre06
	...
	local i  =  449 // (1 + `grupo') * (`canio'-1) 
	
foreach x of numlist 1/2 {
foreach mes of numlist 1/12 {
	mean sexo`x' if mes==`mes'&e30==1 [aw=pesomen] 
	...
	foreach val of numlist 2/3  {
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & bd_region==`val'&e30==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 1/4  {
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & tramo_edad==`val'&e30==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
...
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & e29_1==`val'&e30==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 1/5  {
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & bd_quintilesy==`val'&e30==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 0/1  {
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & pobre06==`val'&e30==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
...
sort ANIO AUXILIAR SEXOJEFE
drop if ANIO==.
drop ANIO_ANT
destring SEXOJEFE, replace
destring AUXILIAR, replace

g CODIND                 = 508
g NOMINDICADOR           = "Distribución porcentual de los hogares según sexo del jefe/a (Varón)" if SEXOJEFE==1
replace NOMINDICADOR     = "Distribución porcentual de los hogares según sexo del jefe/a (Mujer)" if SEXOJEFE==2
g SEXO                   = "NA"
g ASCENDENCIA    		 = "NA"
g QUINTIL       		 = "NA"
g DEPARTAMENTOUY		 = "NA"
...
replace ASCENDENCIA    = "AFRO"        if AUXILIAR==7
replace ASCENDENCIA    = "NO AFRO"     if AUXILIAR==8

replace QUINTIL			 = "QUINTIL 1"							  if AUXILIAR==9
replace QUINTIL			 = "QUINTIL 2"							  if AUXILIAR==10
replace QUINTIL			 = "QUINTIL 3"							  if AUXILIAR==11
replace QUINTIL			 = "QUINTIL 4"							  if AUXILIAR==12
replace QUINTIL			 = "QUINTIL 5"							  if AUXILIAR==13

replace NOMINDICADOR    = NOMINDICADOR+ ". Hogares no en situación de pobreza"    if AUXILIAR==14
replace NOMINDICADOR    = NOMINDICADOR+ ". Hogares en situación de pobreza"       if AUXILIAR==15
...
************************************************************************************************
    ...
    *PAÍS URBANO
	local i      = 1
	local codind = 508                  // Código de indicador
	local grupo  = 10                   // Cantidad de grupos (se agrega un grupo para el total)
	...
	foreach x of numlist 1/2  { 
	mean sexo`x' [aw=bc_pesoan] if bc_filtloc==1&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  0
	matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
	...
	foreach x of numlist 1/2  { 
	foreach val of numlist 1/4  {
	mean sexo`x' [aw=bc_pesoan] if tramo_edad==`val' & bc_filtloc==1&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
    ...
    *
	...
	foreach x of numlist 1/2  { 
	foreach val of numlist 1/5  {
	mean sexo`x' [aw=bc_pesoan] if bd_quintilesy==`val' & bc_filtloc==1&bc_pe4==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    matrix MATR  [`i',4]=  `x'
...
drop uno ypc1 quintilesy_prev
	...
	local i  =  601 // (1 + `grupo') * (`canio'-1) 
	
foreach x of numlist 1/2 {
foreach mes of numlist 1/12 {
	mean sexo`x' if mes==`mes'&e30==1& bc_filtloc==1 [aw=pesomen] 
	matrix aux_2020 [`mes',1]=e(b)
}
	...
	foreach val of numlist 1/4  {
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & tramo_edad==`val'&e30==1& bc_filtloc==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 1/5  {
foreach mes of numlist 1/12 {
	mean sexo`x' [aw=pesomen] if mes==`mes' & bd_quintilesy==`val'&e30==1& bc_filtloc==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
