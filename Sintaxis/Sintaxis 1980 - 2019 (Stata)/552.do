...
* INDICADOR: Tasa de asistencia a la educación de niños/as de 4 a 5 años
* CÓDIGO:    552
...
**MODIFICACIÓN MANUAL: CÓDIGOS Y LOCAL J PARA VARIABLES AUXILIARES**
    ...
    *TOTAL PAÍS
	local i      = 1
	local codind = 552                  // Código de indicador
	...
	keep if bc_pe3>2&bc_pe3<6
	g asiste=1 if bc_pe11==1
	replace asiste=0 if bc_pe11==2
...
rename pobre_06 pobre06
	...
	keep if bc_pe3>2&bc_pe3<6
	g asiste=1 if e49_cv==1
	replace asiste=1 if e49_cv==.&(e193==1|e197==1)
	replace asiste=0 if e49_cv==2
	replace asiste=0 if e49_cv==.&(e193!=1&e197!=1)
	
	local i  =  197 // (1 + `grupo') * (`canio'-1) 
	
foreach mes of numlist 1/12 {
	mean asiste if mes==`mes' [aw=pesomen] 
	...
	foreach val of numlist 2/3  {
foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & bd_region==`val'
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 1/2  {
foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & e26==`val'
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
...
foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & e29_1==`val'
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 1/5  {
foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & bd_quintilesy==`val'
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 0/1  {
foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & pobre06==`val'
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
...
g CODIND                 = 552
g NOMINDICADOR           = "Tasa de asistencia a la educación de niños/as de 3 a 5 años" 
g SEXO                   = "NA"
g ASCENDENCIA    		 = "NA"
g QUINTIL       		 = "NA"
g DEPARTAMENTOUY		 = "NA"
...
************************************************************************************************
    ...
    *PAÍS URBANO
	...
	mean asiste [aw=bc_pesoan] if bc_filtloc==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  0
	matrix MATR  [`i',3]=  `anio'
	...
	foreach val of numlist 1/2  { 
	mean asiste [aw=bc_pesoan] if bc_pe2==`val' & bc_filtloc==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
    ...
    *
	...
	local j  = 	3
	foreach val of numlist 1/5  {
	mean asiste [aw=bc_pesoan] if bd_quintilesy==`val' & bc_filtloc==1
	matrix MATR  [`i',1]=  e(b)
	matrix MATR  [`i',2]=  `j'
    matrix MATR  [`i',3]=  `anio'
...
g bc_pe3=e27
	...
	local i  =  241 // (1 + `grupo') * (`canio'-1) 
	
foreach mes of numlist 1/12 {
	mean asiste if mes==`mes'&bc_filtloc==1 [aw=pesomen] 
	matrix aux_2020 [`mes',1]=e(b)
}
	...
	foreach val of numlist 1/2  {
	foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & e26==`val'&bc_filtloc==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
    ...
    *
	...
	foreach val of numlist 1/5  {
foreach mes of numlist 1/12 {
	mean asiste [aw=pesomen] if mes==`mes' & bd_quintilesy==`val'&bc_filtloc==1
	matrix aux_2020 [`mes',1]=e(b)
}
	matrix MATR  [`i',1]=  (aux_2020[1,1]+aux_2020[2,1]+aux_2020[3,1]+aux_2020[4,1]+ /*
						*/ aux_2020[5,1]+aux_2020[6,1]+aux_2020[7,1]+aux_2020[8,1]+ /*
